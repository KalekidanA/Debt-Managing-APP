 ...
 * (smallest balance first, for behavioral momentum) but supports
 * "avalanche" (highest interest rate first, for mathematically minimal
 * interest paid) as an alternative. */
...
export const PAYOFF_STRATEGY_META: Record<PayoffStrategy, { title: string; summary: string }> = {
  snowball: {
    title: "Debt Snowball",
    summary:
      "Pay off debts smallest balance to largest, regardless of interest rate. Builds momentum with quick wins — Zero's recommended default.",
  },
  avalanche: {
